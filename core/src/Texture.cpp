...
namespace lab {
	...
	void Texture::init(const VulkanContext& ctx, std::string_view path) {
		...
		VkBufferCreateInfo srcBufferCI{ .sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO,
			                            .size = static_cast<VkDeviceSize>(ktx->dataSize),
			                            .usage = VK_BUFFER_USAGE_TRANSFER_SRC_BIT };
		VmaAllocationCreateInfo srcAllocCI{ .flags =
		...
		ctx.submitImmediate([&](VkCommandBuffer cb) {
			...
			vkCmdCopyBufferToImage(cb, srcBuffer, m_Image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, static_cast<uint32_t>(copyRegions.size()),
			                       copyRegions.data());
