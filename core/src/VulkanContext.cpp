...
namespace lab {
	...
	static VKAPI_ATTR VkBool32 VKAPI_CALL vulkanDebugCallback(VkDebugUtilsMessageSeverityFlagBitsEXT severity,
		...
		if (severity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT)
			...
			std::cerr << "[Vulkan Info] "  << data->pMessage;
		else
			std::cerr << "[Vulkan Trace] "  << data->pMessage;
		return VK_FALSE;
	}

	void VulkanContext::init(std::string_view applicationName) {
		initializeSDLAndVolk();
	...
	void VulkanContext::initializeSDLAndVolk() {
		chk(SDL_Init(SDL_INIT_VIDEO));
		chk(SDL_Vulkan_LoadLibrary(NULL));
		volkInitialize();
	}

	void VulkanContext::createInstance(std::string_view applicationName) {
	...
	void VulkanContext::createDebugCallback() {
		...
		if (!vkCreateDebugUtilsMessenger) {
			std::cerr << "Cannot find address of vkCreateDebugUtilsMessenger" << std::endl;
			exit(1);
		}

		chk(vkCreateDebugUtilsMessenger(m_Instance, &ci, nullptr, &m_DebugMessenger));
	}
