...
namespace lab {
    ...
    void VulkanContext::init(std::string_view applicationName) {
        ...
        // Instance
        VkApplicationInfo appInfo{ .sType = VK_STRUCTURE_TYPE_APPLICATION_INFO, .pApplicationName = applicationName.data(), .apiVersion = VK_API_VERSION_1_3 };
        ...
        VkDeviceQueueCreateInfo queueCI{ .sType = VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO, .queueFamilyIndex = m_QueueFamily, .queueCount = 1, .pQueuePriorities = &qfpriorities };
    ...
    void VulkanContext::submitImmediate(const std::function<void(VkCommandBuffer)>& record) const {
        VkCommandPoolCreateInfo poolCI{ .sType = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO, .flags = VK_COMMAND_POOL_CREATE_TRANSIENT_BIT, .queueFamilyIndex = m_QueueFamily };
