...
namespace lab {
    ...
    void VulkanCore::init(std::string_view applicationName) {
        ...
        VkCommandPoolCreateInfo commandPoolCI{ .sType = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO, .flags = VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT, .queueFamilyIndex = m_VulkanCtx.getQueueFamily() };
        chk(vkCreateCommandPool(device, &commandPoolCI, nullptr, &m_CommandPool));
        VkCommandBufferAllocateInfo cbAI{ .sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO, .commandPool = m_CommandPool, .level = VK_COMMAND_BUFFER_LEVEL_PRIMARY, .commandBufferCount = maxFramesInFlight };
    ...
    VkCommandBuffer VulkanCore::beginFrame() {
        ...
        VkResult acquire = vkAcquireNextImageKHR(device, m_Swapchain.getSwapchain(), UINT64_MAX, m_ImageAcquiredSemaphores[m_FrameIndex], VK_NULL_HANDLE, &m_ImageIndex);
        ...
        VkDependencyInfo dep{ .sType = VK_STRUCTURE_TYPE_DEPENDENCY_INFO, .imageMemoryBarrierCount = 2, .pImageMemoryBarriers = toAttachment.data() };
        ...
        VkViewport vp{ .width = static_cast<float>(extent.width), .height = static_cast<float>(extent.height), .minDepth = 0.0f, .maxDepth = 1.0f };
