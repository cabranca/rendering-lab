...
class VulkanApp {
  public:
	~VulkanApp() {
		...
		m_Mesh.destroy(m_Core.getDevice());
	}
  ...
  private:
	...
	void createVertexBuffer() {
		std::vector<lab::Vertex> vertices = {
			...
			{{ -1.f, -1.f, 0.f }, { 0.f, 0.f }}, // Bottom Middle
		};

		m_Mesh.VertexBufferSize = sizeof(lab::Vertex) * vertices.size();
		m_Mesh.VB = m_Core.createVertexBuffer(vertices.data(), m_Mesh.VertexBufferSize);
	}
	...
	void createPipeline() {
		m_GraphicsPipeline.init(m_Core.getDevice(), m_RenderPass, m_Vs, m_Fs, &m_Mesh, m_NumImages);
	}
