...
int main() {
	...
	VkShaderModule vs = loadGLSLShader(device, "assets/shader.vs");
    VkShaderModule fs = loadGLSLShader(device, "assets/shader.fs");
    VkPipelineLayout pipelineLayout = createPipelineLayout(device, descriptorSetLayout);
	VkPipeline pipeline = createGraphicsPipeline(device, pipelineLayout, vs, core.getColorFormat(), core.getDepthFormat());
